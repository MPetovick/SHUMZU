...
class Encryptor:
    """Handles encryption and decryption using AES-GCM."""
...
class Compressor:
    """Handles compression and decompression using Brotli and Zstandard."""
    @staticmethod
...
class QRManager:
    """Handles generation and decoding of QR codes."""
    def __init__(self, encryptor: Encryptor, compressor: Compressor):
    ...
    def generate_qr(self, data: bytes, index: int) -> Image.Image:
        ...
        encrypted = (
            self.encryptor.encrypt(self.compressor.compress(data)) 
            if self.encryptor.password 
            else base64.b64encode(self.compressor.compress(data)).decode()
        )
    ...
    def decode_qr(self, image: Image.Image) -> Dict[int, bytes]:
        ...
        for obj in decoded_objects:
            try:
                ...
                decrypted_data = (
                    self.compressor.decompress(self.encryptor.decrypt(encrypted_data)) 
                    if self.encryptor.password 
                    else self.compressor.decompress(base64.b64decode(encrypted_data))
                )
...
class FileManager:
    """Handles file processing and QR matrix generation/decoding."""
    def __init__(self, qr_manager: QRManager, block_size: int = 1024):
    ...
    def generate_qr_matrix(self, file_path: str, output_path: str):
        ...
        with ThreadPoolExecutor() as executor:
            qr_images = list(tqdm(executor.map(lambda x: self.qr_manager.generate_qr(x[1], x[0]), enumerate([metadata] + blocks)), total=total_blocks, desc="Generating QR"))
    ...
    def decode_qr_matrix(self, image_path: str, output_folder: str):
        ...
        decoded_data = self.qr_manager.decode_qr(Image.open(image_path))
...
def main():
    ...
    qr_manager = QRManager(encryptor, compressor)
    file_manager = FileManager(qr_manager, args.block_size)

    try:
        if args.decode:
            file_manager.decode_qr_matrix(args.file, args.output_folder)
        else:
            file_manager.generate_qr_matrix(args.file, args.output)
    except Exception as e:
